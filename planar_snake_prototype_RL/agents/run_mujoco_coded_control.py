...
def run_environment_episode(env, max_timesteps, render, lambda_deg=50, alpha_deg=60, w_para=0.5, y_para=0.4):
    ...
    # Mean effective length of the snake
    l = 0
    for i in range(K + 1):
    ...
    while (not done) and number_of_timestep < max_timesteps:
        ...
        for i in range(1, K + 1):
            ...
            phi = b * 0 + amp * math.cos(w * t - lam * (i))
            # sim.setJointTargetPosition(carJoints[i], -phi * (1 - math.exp(p * t)))
...
def evaluate_power_velocity(env_id):
    ...
    grid = ParameterGrid(
        param_grid={'lambda_deg': lambda_deg, 'alpha_deg': alpha_deg, 'w_para': w_para, 'y_para': y_para})
    paras = list(grid)
...
def enjoy(env_id):
    ...
    while True:
        ...
        # done, number_of_timesteps, list_infos, dict_list_infos = run_environment_episode_hirose(env,
        # env._max_episode_steps, render)

        print('timesteps of this episode: %d' % (number_of_timesteps))
